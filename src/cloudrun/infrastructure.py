...
def get_bucket_name(sts_client, region: str) -> str:
...
def _create_s3_bucket(s3_client, sts_client, region: str) -> None:
    """Create an S3 bucket if it doesn't exist."""
    bucket_name = get_bucket_name(sts_client, region)
...
def _get_vpc_and_subnet(ec2_client) -> tuple:
...
def create_infrastructure(**kwargs) -> Dict[str, Any]:
    ...
    _create_s3_bucket(aws_clients['s3'], aws_clients['sts'], region)
    ...
    print(f"\n=== CloudRun Infrastructure Creation Complete for environment ===")
