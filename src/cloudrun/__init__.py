...
def get_ecr_repository_url(region: str) -> str:
    """Get the ECR repository URL for the environment."""
    return f"{get_account_id()}.dkr.ecr.{region}.amazonaws.com/cloudrun-executor"
...
def get_bucket_name(region: str, sts_client: boto3.client) -> str:
...
def create_infrastructure(**kwargs) -> Dict[str, Any]:
    ...
    _create_s3_bucket(aws_clients['s3'], region, aws_clients['sts'])
    ...
    ecr_repo = get_ecr_repository_url()
